public class MilkChocolate extends Chocolate {

    private final int milkVolume;

    public MilkChocolate(int sugar, int milkVolume, ServingMethod servingMethod) {
        super(sugar, servingMethod);
        this.milkVolume = milkVolume;
    }

    @Override
    public void drink() {
        servingMethod.drink("milk chocolate beverage");
    }

    @Override
    public void prepare() {
        super.prepare();
        System.out.printf("Put some milk      : %s ml...%n", milkVolume);
        if (sugar > 0) {
            System.out.printf("Put some sugar     : %s pieces ...%n", sugar);
        }
        servingMethod.serve("milk chocolate beverage");
    }

    @Override
    public int cost() {
        return super.cost() + (int) (milkVolume / 20.0);
    }
}
